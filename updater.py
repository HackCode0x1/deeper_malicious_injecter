...
def download_Update():
	cwd = os.getcwd()
	url = 'https://github.com/NoOAYe/deeper_malicious_injecter/archive/master.zip'
	filename = 'masterdeeperinjecter.zip'
	with DownloadProgressBar(unit='B', unit_scale=True,
                             miniters=1, desc='masterdeeperinjecter.zip') as t:
...
def updater():
	...
	Crunt_Version = '4.1.0'
	try:
		Get_Version = urlopen('https://raw.githubusercontent.com/NoOAYe/deeper_malicious_injecter/master/Data/version.txt').read().decode('utf-8').strip()
	...
	if version != Crunt_Version:
		...
		if System=='Linux':
			try:
				cmd('git clone https://github.com/NoOAYe/deeper_malicious_injecter.git')
			except:
				pass
				download_Update()
